...
<div class="tabs" role="tablist" aria-label="Навигация">
    <button class="tab-btn" id="tab-home" role="tab" aria-controls="panel-home" aria-selected="true">Главная</button>
...
<section class="panel" id="panel-home" role="tabpanel" aria-labelledby="tab-home" aria-hidden="false">
    <h2>OAuth 2.0 данные из REQUEST</h2>
    <pre><?php
        // показываем REQUEST для отладки; по необходимости убери
        print_r($_REQUEST);
        ?></pre>
...
<section class="panel" id="panel-files" role="tabpanel" aria-labelledby="tab-files" aria-hidden="true">
    <h2>Загруженные файлы
        <button class="right" onclick="location.reload()">Обновить список</button>
    </h2>
    <p class="muted">Источник: <code><?= htmlspecialchars(__DIR__ . '/uploads/') ?></code></p>
    <?php
    // ===== утилита: получаем список файлов с датами (новые сверху)
    ...
    if (!$filesWithDates) {
        ...
        <form method="post" onsubmit="return confirm('Удалить отмеченные файлы?');">
            <div style="margin-top:10px; display:flex; gap:8px;">
                ...
                <button type="button" onclick="document.querySelectorAll('.file-chk').forEach(cb=>cb.checked=false)">
                    Снять выделение
                </button>
            </div>
